...
def lista_tickets(request: HttpRequest) -> HttpResponse:
    ...
    # Obtenemos todos los tickets, con optimización de consultas
    if request.user.has_perm('tickets.view_ticket'):
        lista_de_tickets = Ticket.objects.select_related(
            'estado', 'herramienta', 'ubicacion', 'creado_por', 'falla'
        ).all().order_by('-fecha_creacion')
    else:
        lista_de_tickets = Ticket.objects.select_related(
            'estado', 'herramienta', 'ubicacion', 'creado_por', 'falla'
        ).filter(creado_por=request.user).order_by('-fecha_creacion')
